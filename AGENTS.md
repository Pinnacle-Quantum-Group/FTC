...
`RLA/...`) refer to files in that *other* repo and only resolve if you
have those sibling repos checked out alongside this one. Paths without a
repo prefix (e.g. `CROSS_REFERENCE.md`, `formal_proofs/...`) are relative
to the root of *this* repo (FTC).
...
| Find a Bekenstein / BH-attractor number                   | `CROSS_REFERENCE.md`                           |
